...
function startSessionBridge(agentName = "cli-agent", sessionId = "s-" + Math.random().toString(36).substring(2, 8), command = "powershell", cmdArgs = []) {
  ...
  const inboxFile = path.join(MESH_DIR, `${safeAgent}.json`);
  const sessionInboxFile = path.join(MESH_DIR, `${safeAgent}-${safeSession}.json`);

  const resolvedMesh = path.resolve(MESH_DIR);
  if (!path.resolve(inboxFile).startsWith(resolvedMesh) || !path.resolve(sessionInboxFile).startsWith(resolvedMesh)) {
    throw new Error("Security Error: Invalid path outside MESH_DIR");
  }

  console.log(`\n=============================================================`);
  console.log(`🚀 [GLOBAL INTERCOM SESSION BRIDGE ACTIVE]`);
  console.log(`👤 Agent Tag     : ${fullAgentTag.toUpperCase()}`);
  console.log(`🆔 Session ID    : ${safeSession}`);
  ...
  const pollInterval = setInterval(() => {
    ...
    if (fs.existsSync(inboxFile)) {
      try {
        ...
        let modified = false;
        allMsgs.forEach(m => {
          if (!m.read && (m.to.toLowerCase() === fullAgentTag || m.to.toLowerCase() === safeAgent)) {
